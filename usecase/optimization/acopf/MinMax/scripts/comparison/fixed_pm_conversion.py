...
def convert_to_pm_structure_bage(net, opf_flow_lim="S", from_time_step=None, to_time_step=None, 
                            **kwargs):
...
#     # use pandapower indexing: 
#     external_bus_numbers = net.bus.index.values         # Pandapower indices + 1
#     internal_indices = ppci['bus'][:, 0].astype(int)   # PyPower bus numbers
...
#         #####
#         #idx = int(row[BUS_I]) + 1 # original
#         idx_pp = int(row[BUS_I])                 # PyPower index
#         idx = internal_to_external[idx_pp] 
#         ####
...
#         pq_mismatch = not np.allclose(pd_value, 0.0, atol=1e-3) or not np.allclose(qd_value, 0.0, atol=1e-3)
#         if pq_mismatch:
#             print(f"PQ mismatch at bus {idx}: pd_value={pd_value:.3f}, qd_value={qd_value:.3f}, "
#             f"pd_bus={pd_bus.get(idx, 0):.3f}, qd_bus={qd_bus.get(idx, 0):.3f}")
#             #raise Exception("PQ mismatch at bus {}. Please report this issue to the pandapower ")
...
#         f_pp = int(row[F_BUS].real)
#         t_pp = int(row[T_BUS].real)
#         branch["f_bus"] = internal_to_external[f_pp]
#         branch["t_bus"] = internal_to_external[t_pp]        
#         ####
...
#     gen_bus_pp = ppci["gen"][:, GEN_BUS].astype(np.int64)   # PyPower indices
#     gen_df["gen_bus"] = [internal_to_external[b] for b in gen_bus_pp]
#     #####
...
#     print(len(pm["load"]), "loads in pm")
#     total_pd_pm = sum(load["pd"] for load in pm["load"].values())
#     total_qd_pm = sum(load["qd"] for load in pm["load"].values())

#     print(f"Total Pd in pm = {total_pd_pm:.6f} pu")
#     print(f"Total Qd in pm = {total_qd_pm:.6f} pu")
#     stop
#     return pm
