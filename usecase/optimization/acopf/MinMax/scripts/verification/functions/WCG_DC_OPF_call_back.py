...
def MILP_WCG(py_init_sed,n_buses, W,b, Gen_delta,Gen_max,config):
    ...
    for n_gcp_iter in range(0,Max_gcp_iter):
                  ...
                  for h in range(len(W)-1):
                        ...
                        for d in range(N_node_per_layer[h+1]):
                            ...
                            if ReLU_tighting[h][d] == 1:
                                ...
                                model.addConstr(xhp1[d]-bh[d]*ReLU_state_h[d] <= gp.quicksum(XLZ[i] for i in range(NPL_h) if Id[i] == 1) + gp.quicksum(UZ[i] for i in range(NPL_h) if Id[i] != 1))
        ...
        logfile = open('./GP_Log/Log_'+str(n_buses) + '_' + str(py_init_sed) + '_' + corelated_input + '_' + BigM_bound + '_' + Ver_type + '_' + Adverse_attack + '_' +str(n_gcp_iter)+'.log', 'w')
