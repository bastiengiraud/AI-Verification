...
def milp_wcg(py_init_sed,n_buses, W,b, Gen_delta,Gen_max,config,Lower_bound, Upper_bound,Pg_min,Pg_max):
    ...
    if BigM_bound != 'None':
        ...
        # Z_min,Z_max = Lower_bound.reshape(Z_min.shape), Upper_bound.reshape(Z_min.shape)
    ...
    """  
    First, compute the worst-case guarantees on generator outputs.
    
    """
    
    base_dir = os.path.dirname(os.path.dirname(os.path.abspath(__file__)))
    output_directory_verif = os.path.join(base_dir, 'functions', str(n_buses))
    os.makedirs(output_directory_verif, exist_ok=True)
    ...
    if BigM_bound == 'MILP':
        ...
        if already_done == False:
            ...
            savetxt(os.path.join(output_directory_verif, 'Gen_max_MILP.csv'), Pg_hat_max, delimiter=',')
            savetxt(os.path.join(output_directory_verif, 'Gen_min_MILP.csv'), Pg_hat_min, delimiter=',')
        else: 
            Pg_hat_min = np.loadtxt(os.path.join(output_directory_verif, 'Gen_min_MILP.csv'), delimiter=',')
            Pg_hat_max = np.loadtxt(os.path.join(output_directory_verif, 'Gen_max_MILP.csv'), delimiter=',')
    elif BigM_bound == 'ABC':      
        ...
        Pg_hat_max = Pg_max.reshape(Pg_hat_max_IB.shape)
        Pg_hat_min = Pg_min.reshape(Pg_hat_min_IB.shape)   
    ...
    """  
    Next, compute the worst-case guarantees on power imbalance.
    ...
    for n_gcp_iter in range(0,Max_gcp_iter):
        ...
        x_min[N_hid_l+1] = 0
        x_max[N_hid_l+1] = 10**5

        y_min[N_hid_l+1] = 1 - 10**5
        ...
        # relu's are added as continuous variables i.e. relaxed LP!
        ...
        # objective is the power balance violation
        m.setObjective(sum(Gen_delta[:,0]*y_hat[i_out])- sum(Dem_min + Dem_delta*x[0]))
        ...
        if Ver_type == 'GCP':
            ...
            while iteration < n_gcp_iter :
                ...
                m_solution={}
                savetxt(os.path.join(output_directory_verif, 'data_compresed.csv'), LP_obj, delimiter=',')
                m_solution['x']={}
                m_solution['y_hat']={}
        ...
        # fix relus to be binary, and solve MILP instead of LP
        for i in range(N_ReLU_layers):
        ...
        if Adverse_attack == 'gd_attack_true' or Adverse_attack == 'Adverse_data_true':
            ...
            if corelated_input == 'correlated':            
                if Adverse_attack == 'gd_attack_true' :
                    ReLU_state_start = Adverse_data_points_correlated(n_buses,W,b,n_adver,Z_min,Z_max,Pg_hat_min,Pg_hat_max,gradient_attak=True)
                else:
                    ReLU_state_start = Adverse_data_points_correlated(n_buses,W,b,n_adver,Z_min,Z_max,Pg_hat_min,Pg_hat_max,gradient_attak=False)        
            else:
                if Adverse_attack == 'gd_attack_true' :
                    ReLU_state_start = Adverse_data_points(n_buses,W,b,n_adver,Z_min,Z_max,Pg_hat_min,Pg_hat_max,gradient_attak=True)
                else:
                    ReLU_state_start = Adverse_data_points(n_buses,W,b,n_adver,Z_min,Z_max,Pg_hat_min,Pg_hat_max,gradient_attak=False)
        ...
        full_log_filename = f"Log_{log_file_name_dynamic_part}.log"
        log_file_path = os.path.join(output_directory_verif, full_log_filename)
...
def ReLU_stability(n_buses,W,b):   
    ...
    data_dir = os.path.join(base_dir, 'DC_OPF_Data', str(n_buses)) 
    dataset_folder_path = os.path.join(data_dir, 'Dataset')
    nn_input_file_path = os.path.join(dataset_folder_path, 'NN_input.csv')

    # Use the constructed path for reading:
    Input_NN = pd.read_csv(nn_input_file_path, header=None)
...
def Adverse_data_points(n_buses,W,b,n_adver,Z_min,Z_max,Pg_hat_min,Pg_hat_max,gradient_attak =True):    
    ...
    # Construct the data directory path
    data_dir = os.path.join(base_dir, 'data/dc_opf_data', str(n_buses)) 
    dataset_folder_path = os.path.join(data_dir, 'Dataset')
    nn_input_file_path = os.path.join(dataset_folder_path, 'NN_input.csv')
    ...
    Adverse_example = Input_NN.iloc[ind]#[:]

    if gradient_attak == True:
        Adverse_example = gradient_attaks(n_buses,W,b,Adverse_example,Z_min,Z_max,Pg_hat_min,Pg_hat_max)
...
def gradient_attaks(n_buses,W,b,X_input,Z_min,Z_max,Pg_hat_min,Pg_hat_max):
